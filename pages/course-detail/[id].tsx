...
export default function CourseDetail({ courseId }) {
  ...
  const isEqualToCourseId = data.reviews[0] && data.reviews[0].courseId && data.reviews[0].courseId === courseId
    ...
    <div className={styles["course-detail-main"]}>
        ...
        <div className={styles["course-detail-head-container"]}>
          <div className={styles["head-instructor-container"]}>
            <div className={styles["instructor-name-container"]}>
              ...
              <div>
                <p>Instructor</p>
                <h2 className={styles["head-instructor-name"]}>{data.instructor.name}</h2>
              </div>
            </div>
      ...
      <Container maxWidth="lg">
        <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
          <Tabs value={currentSectionId} onChange={handleChangeTab} aria-label="basic tabs example">
            <Tab label="Vision General" aria-controls="simple-tabpanel-1" />
            <Tab label="Preguntas Frecuentes" aria-controls="simple-tabpanel-2" />
          ...
          <div className={styles["fyq-main-container"]}>
            <div className={styles["fyq"]}>
              ...
              <div className={styles["course-card-container"]}>
                <CourseDetailCard key={data.courseDetail.id} image={data.courseDetail.image} courseId={courseId} />
              </div>
            </div>
          </div>
        </TabPanel>
