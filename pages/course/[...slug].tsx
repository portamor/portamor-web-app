    ...
    <div className={styles["class-detail-main"]}>
      <div className={styles["video-description-container"]}>
        <div>
          <h1 className={styles["course-title"]}>
            Curso de <span className={styles["course-title-color"]}>{data.courseDetail.title}</span>
          </h1>
          <div className={styles["react-player"]}>
            <ReactPlayer url={data.videoById.videoLink} width="100%" height="100%" onEnded={handleEnd} />
          </div>
        </div>

        <div className={styles["description-container"]}>
          <h2 className={styles["description-title"]}>Descripción del curso</h2>
          <p className={styles["description-text"]}>{data.courseDetail.description}</p>
          <span className={styles["stars-container"]}>
            Valoración del curso:
            {new Array(data.courseDetail.rating).fill(true).map((_, i) => (
          ...
          <div className={styles["buttons-container"]}>
            <Button variant={sectionRightId === 0 ? "contained" : "outlined"} onClick={() => handleSelectChange(0)}>
              Ver temario
            </Button>
            <Button variant={sectionRightId === 1 ? "contained" : "outlined"} onClick={() => handleSelectChange(1)}>
              Sobre el instructor
            </Button>
          </div>
        </div>
      ...
      <div className={styles["secciones-temario-container"]}>
        <div className={styles["buttons-and-cards-container"]}>
          <Tabs value={sectionBottomId} onChange={handleChangeTab} aria-label="basic tabs example">
          ...
          <div className={styles["cards-container"]}>
            ...
            <TabPanel value={sectionBottomId} index={2}>
              {data.courseUsers.length &&
                data.courseUsers.map(user => <UserCard key={user.id} lastName={user.lastName} name={user.name} />)}
            </TabPanel>
